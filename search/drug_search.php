...
<div class="container">
    ...
    <div class="row mt40" style="padding-top: 20px">
        ...
        <table class="table table-bordered">
            ...
            $sq="select * from peptidedrug  where $tit like '%$tcont%'  order by BAPD_ID asc";
            ...
            $sqld="select * from peptidedrug  where $tit like '%$tcont%'  order by BAPD_ID asc limit $start,$pagesize";
            ...
            while($row=mysqli_fetch_assoc($rsd)) {
                ...
                echo ' <tr>
                <td><a href="drugcontent.php?id='.$row['BAPD_ID'].'" >'.$row['BAPD_ID'].'</a></td>
                <td>'.$row['Active_Ingredients'].'</td>
              
                <td>'.$row['Active_Sequence'].'</td>
                <td>'.$row['Type'].'</td>
               <td>'.$row['Disease'].'</td>
            </tr>';
            }
...
<div class="container mt-5">
    ...
    <div class="text-right mb-5">
        ...
        echo ' <a href="drug_search.php?page=1&slt='.$tit.'&txtarea='.$tcont.'" class="btn btn-default">First</a> ';
        ...
        if($pagecount>$showpages){
            ...
            for($i=$start;$i<=$end;$i++){
                if($i==$page){
                    echo ' <a href="drug_search.php?page='.$i.'&slt='.$tit.'&txtarea='.$tcont.'" class="btn btn-primary">'.$i.'</a> ';
                }else{
                    echo ' <a href="drug_search.php?page='.$i.'&slt='.$tit.'&txtarea='.$tcont.'" class="btn btn-default" >'.$i.'</a> ';
                }
            }
        }else{
            for($i=$start;$i<=$end;$i++) {
        ...
        echo ' <a href="drug_search.php?page='.$pagecount.'&slt='.$tit.'&txtarea='.$tcont.'" class="btn btn-default">Last</a> ';
